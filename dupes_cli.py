...
def _parse_size_bytes(song: Dict[str, Any]) -> int:
    ...
    try:
        ...
        pass
    m = re.search(r"([\d.]+)\s*(B|KB|MB|GB)", str(song.get("size_formatted") or "").upper())
    if not m:
        return 0
...
def get_device_filename_set(device_serial: Optional[str], redis_cfg: Optional[Dict[str, Any]]) -> Set[str]:
    """
    ...
    if not device_serial:
        return set()
    try:
        raw = adb_manager.query_songs_from_device(device_serial, redis_cfg=redis_cfg, refresh_cache=False)
        ...
        logger.info(f"[Dupes] Loaded {len(names)} device filenames from [{device_serial}] — used to prefer copies on device.")
        return names
    except Exception as e:
        logger.warning(f"[Dupes] Could not query device [{device_serial}] for keeper preference: {e}")
        return set()


def pick_keeper(cluster_songs: List[Dict[str, Any]], device_filename_set: Set[str]) -> Dict[str, Any]:
    """
    ...
    def key(s: Dict[str, Any]) -> Tuple:
        on_dev = int(_norm(os.path.basename(s.get("filepath") or "")) in device_filename_set)
        return (on_dev, _parse_bitrate(s), _parse_size_bytes(s), float(s.get("mtime") or 0))

    best = sorted(cluster_songs, key=key, reverse=True)[0]
    keep_path = os.path.abspath(best.get("filepath") or "")
    same_path = [s for s in cluster_songs if os.path.abspath(s.get("filepath") or "") == keep_path]
...
def plan_deletions(dups: Dict[str, Any], device_filename_set: Set[str]):
    ...
    for cluster in dups.get("clusters") or []:
        ...
        deletes = [s for s in songs if os.path.abspath(s.get("filepath") or "") != keeper_path]
...
def _cluster_blocks(plan: List[Dict[str, Any]], device_filename_set: Set[str]) -> List[str]:
    ...
    for idx, item in enumerate(plan, 1):
        ...
        for j, s in enumerate(songs, 1):
            ...
            on_dev = "*ON-DEVICE*" if (_norm(os.path.basename(path)) in device_filename_set) else ""
            mark = "*" if is_keep else " "
            meta = f"{s.get('size_formatted') or _format_bytes(_parse_size_bytes(s))} | {s.get('bitrate_kbps') or 'Unknown'}" \
        ...
        lines.append(f"      -> keep: {os.path.basename(keeper_path)} ({item['keep'].get('size_formatted') or ''})")
        blocks.append("\n".join(lines))
    return blocks


def print_report(plan: List[Dict[str, Any]], device_filename_set: Set[str], total_dup: int = 0, freed: int = 0):
    """Print the duplicate cleanup plan report without deleting anything."""
    print("\n" + "=" * 70)
    print(f"            DUPLICATE FILES REPORT — {len(plan)} cluster(s), {total_dup} duplicate file(s)")
    print("=" * 70)
    for block in _cluster_blocks(plan, device_filename_set):
        print()
        print(block)
    print()
    print(f"Summary: {len(plan)} cluster(s) — {total_dup} duplicate file(s), "
          f"~{_format_bytes(freed)} of duplicates would be moved to trash.")
    print("No files deleted. Use --dupes -d to delete automatically, or --dupes -i interactively.\n")


def run_interactive(plan: List[Dict[str, Any]], device_filename_set: Set[str]) -> List[str]:
    """
    ...
    for idx, item in enumerate(plan, 1):
        ...
        while True:
            try:
                choice = input(f"  Keep copy [1-{len(songs)}] {default_msg} | [s]kip cluster | [q]uit: ").strip().lower()
            except (KeyboardInterrupt, EOFError):
                print("\nAborted by user.")
                return deletes

            if choice in ("s", "skip"):
                print(f"  Skipping cluster — nothing will be deleted from '{cluster.get('cluster_name', '')}'.")
                break
            if choice in ("q", "quit"):
            ...
            if choice.isdigit():
                ...
                if 1 <= chosen <= len(songs):
                    chosen_path = os.path.abspath(songs[chosen - 1].get("filepath") or "")
                    for s in songs:
                        p = os.path.abspath(s.get("filepath") or "")
                        if p != chosen_path and p not in deletes:
                            deletes.append(p)
                    n = len(songs) - 1
                    print(f"  Marked {n} cop{'y' if n == 1 else 'ies'} for deletion (keeping "
...
def _delete_paths(paths: List[str], auto_confirm: bool = False) -> Dict[str, Any]:
    ...
    if not auto_confirm and not _confirm("Proceed to delete these duplicates? [Y/n]: "):
        ...
        return {"status": "cancelled", "deleted_count": 0, "failed": [], "message": "Cancelled by user"}
...
def run_dupes_workflow(
    ...
    if interactive and immediate_delete:
        print("[Dupes] Error: -i (interactive) and -d (immediate delete) cannot be used together.")
        sys.exit(1)
    ...
    print("=" * 70)
    print(f"[Dupes] Detection mode: {'acoustic audio fingerprinting' if use_fingerprint else 'tag & filename matching'}", flush=True)
