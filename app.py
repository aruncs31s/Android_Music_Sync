...
def main():
    ...
    if args.list_hidden:
        ...
        if not records:
            logger.info("No hidden files in SQLite database (sync_hide_list.db).")
        else:
            logger.info(f"Hidden Files in SQLite Database ({len(records)} entries):")
            for r in records:
                logger.info(f"  [{r['id']}] {r['filename']} | Path: {r['filepath']} (Hidden at: {r['hidden_at']})")
        return
    ...
    if args.list_synced:
        ...
        if not records:
            ...
            logger.info(f"No synced track history found{dev_str} in SQLite database (sync_hide_list.db).")
        else:
    ...
    if args.list_devices:
        try:
            ...
            if not devices:
                ...
                logger.info(f"Connected ADB devices ({len(devices)}):")
                for idx, dev in enumerate(devices, 1):
                    logger.info(f"  [{idx}] {dev['serial']} | Model: {dev['model']} | State: {dev['state']}")
    ...
    if args.file:
        ...
        if not devices:
            logger.error("No ADB devices connected. Please connect an Android device via USB/Wi-Fi.")
            sys.exit(1)
        ...
        logger.info(f"Querying songs from ADB device {target_device['description']}...")
        try:
            ...
            logger.error(f"Failed to query songs from device: {e}")
            sys.exit(1)

    logger.info(f"Loaded {len(songs)} songs from {source_name}.")

    if not songs:
        logger.info("No music files found in specified source.")
        sys.exit(0)
